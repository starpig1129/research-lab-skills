...
研究には２つの異なる側面があり、互いに対話することはほとんどありません。

１つ目は**日々のプロセス**——昨夜走らせた実験、ようやく意味をなした図、アーキテクチャを変更することを決めた午後、金曜日のラボミーティングのスライド。この層はほとんどの論文では見えませんが、実際の思考が起きる場所です。

２つ目は**正式なアウトプット**——系統的文献レビュー、3回の査読を乗り越えた論文草稿、主張を実際に支持する引用。この層は知識が積み重なる場所ですが、混乱しながらも真実の研究プロセスから切り離されがちです。

私はもともと **Lab スキル**（`research-log`、`report-slides`、`research-mode`）を最初の層を捉えるために構築しました。[Cheng-I Wu](https://github.com/Imbad0202) は独立して **Academic Research Skills（ARS）** を構築し、2番目の層を体系化しました。

Lab スキルは*どこに行ったか*を記録し、ARS は*どこに向かうか*を構造化します。実験日誌は論文の方法論セクションを育て、進捗スライドは草稿の図表になります。これら２つのシステムは単に並べて置かれているのではなく、同じ研究者が同じプロジェクトの異なるフェーズに向けて設計されています。
