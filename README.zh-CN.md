...
研究有两段截然不同的生命，而它们很少真正对话。

第一段是**每日的过程**——昨夜跑完的实验、那张终于说得通的图、决定换架构的那个下午、周五 lab meeting 的幻灯片。这一层在绝大多数论文里是隐形的，但它才是真正思考发生的地方。

第二段是**正式的产出**——系统性文献回顾、熬过三轮审稿的论文草稿、真正支撑主张的引用。这一层是知识积累的地方，但往往与那些混乱却真实的研究过程脱节。

我最初建立 **Lab 技能**（`research-log`、`report-slides`、`research-mode`）是为了捕捉第一层：一个结构化的实验日志、一个能把日志转成进度幻灯片的 SVG 生成器，以及一套工作模式让你知道自己现在在做哪种研究。

[Cheng-I Wu](https://github.com/Imbad0202) 独立建立了 **Academic Research Skills（ARS）** 来系统化第二层：一个有 PRISMA 支持与四索引引用验证的 13-agent 深度研究引擎、一个有引用 anchor 与图表忠实度查验的 12-agent 论文撰写系统，以及把它们串接起来的 10 阶段 pipeline 协调器。

当我发现 ARS 时，两者的互补性是立即可见的。Lab 技能记录*你去过哪里*；ARS 结构化*你要去哪里*。这两个系统不是并排放在一起——它们为同一位研究者在同一个项目的不同阶段而设计。
