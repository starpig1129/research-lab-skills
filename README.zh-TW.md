...
研究有兩段截然不同的生命，而它們很少真正對話。

第一段是**每日的過程**——昨夜跑完的實驗、那張終於說得通的圖、決定換架構的那個下午、週五 lab meeting 的投影片。這一層在絕大多數論文裡是隱形的，但它才是真正思考發生的地方。

第二段是**正式的產出**——系統性文獻回顧、熬過三輪審稿的論文草稿、真正支撐主張的引用。這一層是知識累積的地方，但往往與那些混亂卻真實的研究過程脫節。

我最初建立 **Lab 技能**（`research-log`、`report-slides`、`research-mode`）是為了捕捉第一層：一個結構化的實驗日誌、一個能把日誌轉成進度簡報的 SVG 生成器，以及一套工作模式讓你知道自己現在在做哪種研究。目標是讓研究*過程*變得可讀——對自己、對指導教授、對合作者。

[Cheng-I Wu](https://github.com/Imbad0202) 獨立建立了 **Academic Research Skills（ARS）** 來系統化第二層：一個有 PRISMA 支援與四索引引用驗證的 13-agent 深度研究引擎、一個有引用 anchor 與圖表忠實度查驗的 12-agent 論文撰寫系統，以及把它們串接起來的 10 階段 pipeline 協調器。

當我發現 ARS 時，兩者的互補性是立即可見的。Lab 技能記錄*你去過哪裡*；ARS 結構化*你要去哪裡*。實驗日誌餵養論文的方法論章節。進度簡報成為草稿的圖表。工作模式（`exp` → `explore` → `publish`）直接對應 ARS 設計的研究生命週期。

這兩個系統不是並排放在一起——它們為同一位研究者在同一個專案的不同階段而設計。把它們合進一個 repo 一條安裝指令，是最自然的結論。
